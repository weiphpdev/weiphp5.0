...
`price`  decimal(10,2) NULL  DEFAULT 0 COMMENT '商品价格',
...
`template`  char(10) NULL  DEFAULT 1 COMMENT '模板',
`need_subscribe`  tinyint(2) NULL  DEFAULT 0 COMMENT '关注公众号才能参与',
`need_member`  tinyint(2) NULL  COMMENT '成为会员才能参与',
`show_prize_num`  tinyint(2) NULL  DEFAULT 0 COMMENT '是否显示奖品数量',
`show_winner`  tinyint(2) NULL  DEFAULT 0 COMMENT '是否显示中奖记录',
`show_prize_details`  tinyint(2) NULL  DEFAULT 0 COMMENT '是否显示奖品详情',
...
`current_draw_num`  int(10) NULL  DEFAULT 0 COMMENT '当前抽奖次数',
`draw_count`  int(10) NULL  DEFAULT 0 COMMENT '活动抽奖总次数',
...
`explain`  text NULL  COMMENT '奖品说明',
`coupon_id`  char(50) NULL  COMMENT '选择赠送券',
`money`  decimal(10,2) NULL  COMMENT '返现金额',
`sort`  int(10) unsigned NULL  DEFAULT 0 COMMENT '排序号',
`uid`  int(10) NULL  COMMENT 'uid',
`wpid`  int(10) NOT NULL  DEFAULT 0 COMMENT 'wpid',
`aim_table`  varchar(255) NULL  COMMENT '活动标识',
