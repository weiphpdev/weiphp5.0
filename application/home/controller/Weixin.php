...
class Weixin extends Home
{
    ...
    private function reply($data, $weixin)
    {
        ...
        if (is_install('message_push')) {
            ...
            $uid = $uid <= 0 ? 0 : $uid;
            $type = -1;
            if ($data ['MsgType'] == 'text' && !empty ($data ['Content'])) {
                $type = 1; // 会话
            } elseif ($data ['MsgType'] == 'event' && strtolower($data ['Event']) == 'subscribe') {
                $type = 0; // 关注
            } elseif ($data ['MsgType'] == 'event' && strtolower($data ['Event']) == 'scan') {
                $type = 2; // 扫码
            } elseif ($data ['MsgType'] == 'event' && strtolower($data ['Event']) == 'click') {
                $type = 3; // 点菜单
            }
            if ($type > -1) {
                // 保存记录
                D('message_push/MessagePush')->addFans($type, $uid, $data ['FromUserName']);
            }
        }
